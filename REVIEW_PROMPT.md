...
Follow REVIEW.md's new-model promotion procedure: establish a release baseline, then propose one higher model tier per eligible role for each distinct qualifying introduction, preserving exact reasoning levels and recording fixed per-role targets to prevent repeated promotion. The starting ladder is Luna → Sol → Astra; the highest tier stays unchanged until a higher compatible model exists. For ordinary workload tuning, recommend at most one evidence-backed adjustment. Run at most three small validation checks per review in an isolated workspace, leaving unvalidated promotion targets pending. Never use production mutations, paid APIs, reset credits, or relaxed security settings to run tests. Preserve required verification and explicit user preferences. If compatibility evidence is weak, keep the incumbent.
