...
4. Initial `python sync.py --adopt` installs the automatic updater. Verify its task, systemd timer, or crontab fallback and a sync run. Use `--no-schedule` only when the user explicitly opts out. This configures only Git synchronization, not a model-consuming review. Preserve auth, MCP settings, permissions, projects, histories, and unrelated instruction blocks. Do not install ADHD skills.
